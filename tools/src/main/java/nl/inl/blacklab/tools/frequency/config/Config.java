...
public record Config(
    ...
    public Config(final File outputDir, final Config config) {
        this(outputDir, config.runConfig, config.frequencyLists);
    }

    /**
     * Read config from file.
     */
    public static Config fromFile(final File file, final File outputDir) {
        try {
            final ObjectMapper mapper = new ObjectMapper(new YAMLFactory());
            final var cfg = mapper.readValue(file, Config.class);
            return new Config(outputDir, cfg);
