...
public record AnnotationHelper(
    ...
    public static AnnotationHelper create(final BlackLabIndex index, final FrequencyListConfig cfg) {
        ...
        final var forwardIndices = annotations.stream().map(index::forwardIndex).toList();
        final var sensitivities = cfg.annotations().stream().map(AnnotationConfig::sensitivity).toList();
        return new AnnotationHelper(annotatedField, annotations, forwardIndices, sensitivities);
