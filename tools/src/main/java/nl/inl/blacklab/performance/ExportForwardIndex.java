...
public class ExportForwardIndex {
    ...
    public static void main(String[] args) throws ErrorOpeningIndex, IOException {
        ...
        try (BlackLabIndex index = BlackLab.open(indexDir)) {
            ...
            if (doLengths || doTokens)
                exportDocs(index, annotatedField, doLengths, doTokens);
        }
    }

    private static void exportDocs(BlackLabIndex index, AnnotatedField annotatedField, boolean doLengths, boolean doTokens) {
    ...
    private static void exportTerms(BlackLabIndex index, AnnotatedField annotatedField) {
        ...
        for (Annotation annotation: annotatedField.annotations()) {
            ...
            for (String term: allTerms) {
                System.out.printf("    %03d %s%n", i, term);
                i++;
            }
        }
    }
