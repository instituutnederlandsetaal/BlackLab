...
| `scorer`                           | (from BlackLab 5.0 / current `dev`) How to score hit groups. See [Collocations](collocations#scorers)                                                                                                                                                     |
...
| `subcorpussize`      | Whether to include the size of the subcorpus defined by the `filter` query in the `summary.results.stats` section. Note that if you're grouping by metadata, each group will also contain `subcorpusSize` for that group specifically. Default: `false` (NOTE: this parameter used to be called `includetokencount`)                                                                                                                                                                              |
