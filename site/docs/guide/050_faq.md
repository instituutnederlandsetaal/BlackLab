...
- [BlackLab Corpus Query Language](/guide/query-language/) (BCQL), similar to the query languages of the IMS Corpus WorkBench and Lexical Computing Sketch Engine;
...
The topic of AI / machine learning is discussed a lot nowadays. Let's clarify how (and if) we use it in BlackLab.

Search in BlackLab does not use AI. It is intended, among others, for scientists, so precise, reproducible results are important. A user specifies what they want to search for, and BlackLab finds it exactly as specified. Users can easily search for one or more words, and there is a [query language](/guide/query-language/) to express more complex search requests.

Because not everyone is familiar with this query language, we may develop an (optional) feature where you can input what you are looking for ("find the most used adjectives that start with a b") which is then converted into a query for you. The user sees the query and can execute it. That feature would use generative AI (trained on example queries written by us), but it can be disabled if desired.
...
Programmers working on BlackLab may use AI-based tools (such as GitHub Copilot) to help them write code more efficiently. This can be compared to using autocomplete features in an IDE or on your phone. It does not replace the programmer's thinking, but it can save time on typing and looking up syntax. Most programmers use such tools nowadays.
