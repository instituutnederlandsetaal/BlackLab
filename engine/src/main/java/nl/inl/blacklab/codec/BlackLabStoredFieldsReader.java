...
public abstract class BlackLabStoredFieldsReader extends StoredFieldsReader {
    ...
    /** Whether we need to reverse the endianness of the data. */
    protected final boolean reverseEndian;

    public BlackLabStoredFieldsReader(String storedFieldsFormatName, int minVersion, int maxVersion,
            Directory directory, SegmentInfo segmentInfo, IOContext ioContext, FieldInfos fieldInfos,
            StoredFieldsReader delegate, String delegateFormatName, boolean reverseEndian) throws IOException {
        this.storedFieldsFormatName = storedFieldsFormatName;
     ...
     */
    protected IndexInput openInput(String extension, Directory directory, SegmentInfo segmentInfo, IOContext ioContext) throws IOException {
        ...
        try {
            // Check index header
            String codecName = storedFieldsFormatName + "_" + extension;
            CodecUtil.checkIndexHeader(input, codecName, minVersion,
                    maxVersion, segmentInfo.getId(), segmentSuffix);
    ...
    public void document(int docId, StoredFieldVisitor storedFieldVisitor) throws IOException {
        ...
        for (FieldInfo fieldInfo: fieldInfos) {
            ...
            case YES:
                // Visit this field.
                if (BlackLabIndexImpl.isContentStoreField(fieldInfo)) {
    ...
    public synchronized ContentStoreSegmentReader contentStore() {
        ...
        return new ContentStoreSegmentReaderImpl();
    }

    private class ContentStoreSegmentReaderImpl implements ContentStoreSegmentReader {
        ...
        private int findBlockStartOffset(long blockIndexOffset, long blocksOffset, int blockNumber)
            ...
            if (blockNumber == 0) {
                ...
                blockIndexFile.seek(blockIndexOffset + (long) (blockNumber - 1) * BlackLabStoredFieldsReader.BLOCKINDEX_RECORD_SIZE);
        ...
        private int findValueLengthChar(int docId, String luceneField) throws IOException {
            ...
            // Find the document
            docIndexFile.seek(docIndexFileOffset + (long) docId * BlackLabStoredFieldsReader.DOCINDEX_RECORD_SIZE);
            ...
            while (i < numberOfContentStoreFields) {
                valueIndexFile.seek(valueIndexOffset + (long) i * BlackLabStoredFieldsReader.VALUEINDEX_RECORD_SIZE);
